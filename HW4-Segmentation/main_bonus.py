...
def draw_circle(event,x,y,flags,param):
    global ix,iy,drawing,mode,mouseCall
    # print(mouseCall)
    if mouseCall == True:
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if drawing == True:
                if mode == True:
                    cv2.circle(imgdraw, (x, y), 5, (0, 0, 255), -1)
                    cv2.circle(imgMask, (x, y), 5, (0, 0, 255), -1)
                else:
                    cv2.circle(imgdraw,(x,y), 5,(255,0,0),-1)
                    cv2.circle(imgMask,(x,y), 5,(255,0,0),-1)
            ...
            if mode == True:
                ...
                cv2.circle(imgdraw,(x,y), 7,(255,0,0),-1)
                cv2.circle(imgMask,(x,y), 7,(255,0,0),-1)

# Calculate the SLIC superpixels, their histograms and neighbors
def superpixels_histograms_neighbors(img):
    # SLIC
    # #TUNE THIS
    segments = slic(img, n_segments=500, compactness=18.4534534535)
...
if __name__ == '__main__':
    ...
    imgMask = np.ones(img.shape, np.uint8)*255

    #drawing window and callback
    cv2.namedWindow('image')
    cv2.setMouseCallback('image', draw_circle)

     ##use c to change color for background and foreground marking and then press escape for segmenting
    while 1:
        ...
        if k == ord('c') or k == ord('C'):
            ...
            # cv2.imwrite('drawing.png', imgMask)
    ...
    centers, colors_hists, superpixels, neighbors = superpixels_histograms_neighbors(img)
    # cv2.imshow('mask', mark_boundaries(img,superpixels))
    # cv2.waitKey(0)

    #exit if no fg and bg is present
    if ( np.where(img_marking[:,:,0]!=255)[0].size==0 or np.where(img_marking[:,:,2]!=255)[0].size==0):
        exit(1)

    fg_segments, bg_segments = find_superpixels_under_marking(img_marking, superpixels)

    fgbg_hists = list()
    fgbg_hists.append(cumulative_histogram_for_superpixels(fg_segments,colors_hists))
    fgbg_hists.append(cumulative_histogram_for_superpixels(bg_segments,colors_hists))

    fgbg_superpixels = list()
    ...
    selection=np.where(graph_cut)
    # print(selection)
    bool_mask=pixels_for_segment_selection(superpixels,selection)
    # print(bool_mask.shape)

    mask = np.zeros((img.shape[0],img.shape[1]),dtype=np.uint8)
    mask[np.where(bool_mask)] = 255

    cv2.imshow('maskOut',mask)
    cv2.waitKey(0)

    # master = cv2.imread('example_output.png',cv2.IMREAD_GRAYSCALE)
    # print(master.shape)
    # print(mask.shape)
    # print(RMSD(mask,master))
